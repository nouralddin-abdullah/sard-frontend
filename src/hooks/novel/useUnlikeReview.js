...
const unlikeReview = async ({ novelId, reviewId }) => {
	...
	const response = await fetch(`${BASE_URL}/api/${novelId}/reviews/${reviewId}/unlike`, {
		method: "DELETE",
		headers,
	});
	...
	if (!response.ok) {
		...
		throw new Error(errorData.message || "Failed to unlike review");
	}
