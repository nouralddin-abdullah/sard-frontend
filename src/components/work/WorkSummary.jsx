...
const insightCards = [
  {
    ...
    accent: "bg-gradient-to-br from-blue-600/25 to-indigo-500/25 border-blue-400/40",
    icon: Sparkles,
  },
  {
    key: "cadence",
    ...
    accent: "bg-gradient-to-br from-purple-600/20 to-violet-500/20 border-purple-400/40",
    icon: Clock3,
  },
];
...
const WorkSummary = ({ works = [], isLoading = false }) => {
  ...
  const ongoingCount = works.filter((work) =>
    work?.status?.toLowerCase() === "ongoing"
  ).length;
  const completedCount = works.filter((work) =>
    work?.status?.toLowerCase() === "completed"
  ).length;
  const hiatusCount = works.filter((work) =>
    work?.status?.toLowerCase() === "hiatus"
  ).length;
  const totalViews = works.reduce((acc, work) => acc + (work.totalViews || 0), 0);
    ...
    <section className="grid gap-6 lg:grid-cols-[2fr_1.1fr]">
      <div className="rounded-[28px] border border-zinc-700/70 bg-zinc-900/80 px-8 py-7 shadow-[0_20px_55px_-35px_rgba(0,0,0,0.7)]">
        <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-zinc-500">Portfolio health</p>
            <h2 className="text-2xl font-semibold text-zinc-50 mt-1">
              Your pipeline at a glance
            </h2>
          </div>
          <div className="flex items-center gap-2 text-xs text-zinc-400">
            <ArrowUpRight className="w-3 h-3" /> Updated {isLoading ? "just now" : lastUpdated}
          </div>
        </header>

  <div className="mt-8 grid gap-6 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
          <StatBlock label="Total works" value={isLoading ? "—" : totalWorks} caption="All projects" />
          <StatBlock label="Ongoing" value={isLoading ? "—" : ongoingCount} caption="In production" />
          <StatBlock label="Completed" value={isLoading ? "—" : completedCount} caption="Ready to binge" />
          <StatBlock label="On pause" value={isLoading ? "—" : hiatusCount} caption="Hiatus or planning" />
          <StatBlock
      ...
      <div className="grid gap-4">
        {insightCards.map(({ key, title, description, accent, icon: Icon }) => (
          ...
          >
            ...
            <p className="mt-3 text-sm text-zinc-200/90 leading-relaxed">{description}</p>
          </article>
        ))}
      </div>
    </section>
    ...
    <div className="rounded-2xl border border-zinc-700 bg-zinc-900/60 px-5 py-4">
      <div className="flex items-center gap-3">
        <div className="flex-1">
          <p className="text-sm text-zinc-400 uppercase tracking-[0.25em]">{label}</p>
